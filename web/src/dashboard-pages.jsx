...
export const DashboardSidebar = ({ admin = false, actorName = "", logout }) => {
...
const UserShell = ({ session, logout, title, children }) => {
  ...
  if (!session?.token || session.role !== "user") return <div className="dashboard-shell"><DashboardSidebar logout={logout} /><section className="dashboard-content"><DashboardGate message="Sign in with a user account to load live helmet data." /></section></div>;
  return <div className="dashboard-shell"><DashboardSidebar actorName={profile?.fullName || session.actor?.fullName || ""} logout={logout} /><section className="dashboard-content"><h1>{title}</h1><FormStatus status={error ? { type: "error", text: error } : null} />{children}</section></div>;
};

const AdminShell = ({ session, logout, title, children, status }) => {
  if (!session?.token || session.role !== "admin") return <div className="dashboard-shell"><DashboardSidebar admin logout={logout} /><section className="dashboard-content"><DashboardGate message="Sign in with an admin account to load live admin data." /></section></div>;
  return <div className="dashboard-shell"><DashboardSidebar admin actorName={session.actor?.name || ""} logout={logout} /><section className="dashboard-content"><h1>{title}</h1><FormStatus status={status} />{children}</section></div>;
};
...
export const UserDashboard = ({ session, logout }) => {
  ...
  return <div className="dashboard-shell"><DashboardSidebar actorName={profile?.fullName || session.actor?.fullName || ""} logout={logout} /><section className="dashboard-content"><h1>Dashboard</h1><FormStatus status={error ? { type: "error", text: error } : requestStatus.text ? requestStatus : null} /><div className="stats-grid four riding-grid"><StatCard icon="ST" tone="green" hint={helmet.isActive == null ? "" : helmet.isActive ? "Active" : "Inactive"} title="Helmet Status" value={lastStatus.online == null ? "" : lastStatus.online ? "Online" : "Offline"} /><StatCard icon="RM" tone="green" hint={helmet.ridingModeActive ? `Helmet ID: ${helmet.helmetId || helmet.espId || ""}` : ""} title="Riding Mode" value={helmet.ridingModeActive == null ? "" : helmet.ridingModeActive ? "ON" : "OFF"} /><StatCard icon="BT" hint={blank(helmet.firmwareVersion)} title="Battery" value={lastStatus.batteryPercentage == null ? "" : `${lastStatus.batteryPercentage}%`} /><StatCard icon="GP" tone="red" hint={lastStatus.gpsSignal == null ? "" : lastStatus.gpsSignal ? "Live" : "No lock"} title="GPS Location" value={blank(helmet.label)} /><StatCard icon="LS" tone="purple" hint={connection === "connected" ? "Connected" : connection === "error" ? "Error" : ""} title="Realtime Socket" value={connection === "connected" ? "Connected" : ""} /></div><div className="highlight-grid"><div className="highlight-card blue"><h3>Emergency Contacts</h3><strong>{contacts.length}</strong></div><div className="highlight-card orange"><h3>Accident Logs</h3><strong>{accidents.length}</strong></div><div className="highlight-card green"><h3>Connection</h3><strong>{lastStatus.online == null ? "" : lastStatus.online ? "Stable" : "Offline"}</strong></div></div><div className="panel-card"><div className="row-between"><div><h2>Emergency SOS</h2><p>Use the helmet button or app workflow to trigger an immediate emergency alert to your configured contacts.</p></div><button className="btn back-btn" type="button" onClick={requestLocation}>Request Live Location</button></div><button className="btn danger sos-btn" type="button">SOS - Send Emergency Alert</button>{latestResolved?.request?.lastKnownLocation ? <div className="detail-stack"><small>Latest realtime location</small><strong>{latestResolved.request.lastKnownLocation.mapUrl || `${latestResolved.request.lastKnownLocation.lat}, ${latestResolved.request.lastKnownLocation.lng}`}</strong></div> : null}</div><div className="panel-card tall"><h2>Live Helmet Activity</h2><div className="map-box">Real-time GPS Map<br /><small>{lastStatus.gpsSignal == null ? "" : lastStatus.gpsSignal ? "GPS signal available" : "No GPS signal"}</small><small>{helmet.ridingModeActive == null ? "" : helmet.ridingModeActive ? "Riding mode active for this helmet" : ""}</small></div></div></section></div>;
};
...
export const UserHelmetPage = ({ session, logout }) => {
  ...
  return <UserShell session={session} logout={logout} title="My Helmet"><FormStatus status={statusMessage.text ? statusMessage : null} /><div className="row-between"><div className="subtle">Realtime socket: {connection}</div><button className="btn primary" type="button" onClick={requestLocation}>Request Live Location</button></div><div className="split-panels"><div className="panel-card"><h2>Helmet Information</h2><div className="info-table"><div><span>Helmet ID</span><strong>{blank(helmet.helmetId || helmet.espId)}</strong></div><div><span>Model</span><strong>{blank(helmet.firmwareVersion)}</strong></div><div><span>Name</span><strong>{blank(helmet.label)}</strong></div><div><span>Status</span><strong>{helmet.isActive == null ? "" : helmet.isActive ? "Active" : "Inactive"}</strong></div><div><span>Battery</span><strong>{status.batteryPercentage == null ? "" : `${status.batteryPercentage}%`}</strong></div><div><span>WiFi Connected</span><strong>{status.online == null ? "" : status.online ? "Yes" : "No"}</strong></div><div><span>Last Updated</span><strong>{status.createdAt ? new Date(status.createdAt).toLocaleTimeString() : ""}</strong></div><div><span>Latest Location</span><strong>{latestResolved?.request?.lastKnownLocation?.mapUrl || ""}</strong></div></div></div><div className="panel-card"><h2>Sensor Status</h2><div className="sensor-list"><div className="sensor-row"><span>Shock Sensor</span><strong>{status.sensors?.shock == null ? "" : status.sensors.shock ? "Active" : "Off"}</strong></div><div className="sensor-row"><span>GPS Tracker</span><strong>{status.gpsSignal == null ? "" : status.gpsSignal ? "Active" : "Waiting"}</strong></div><div className="sensor-row"><span>Bluetooth</span><strong>{status.sensors?.bluetooth == null ? "" : status.sensors.bluetooth ? "Connected" : "Disconnected"}</strong></div><div className="sensor-row"><span>Wear Detection</span><strong>{status.helmetWorn == null ? "" : status.helmetWorn ? "On" : "Off"}</strong></div></div></div></div></UserShell>;
};
...
export const UserAccidentsPage = ({ session, logout }) => {
  ...
  return <UserShell session={session} logout={logout} title="Accident History"><div className="panel-card"><table className="data-table"><thead><tr><th>Date</th><th>Time</th><th>Location</th><th>Severity</th><th>Status</th></tr></thead><tbody>{accidents.map((item) => <tr key={item._id}><td>{item.createdAt ? new Date(item.createdAt).toLocaleDateString() : ""}</td><td>{item.createdAt ? new Date(item.createdAt).toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" }) : ""}</td><td>{item.location?.mapUrl || ""}</td><td><span className={`status-pill ${item.severity === "severe" ? "danger" : item.severity === "medium" ? "warn" : "ok"}`}>{item.canceled ? "False Alarm" : blank(item.severity)}</span></td><td>{blank(item.responseStatus)}</td></tr>)}</tbody></table></div></UserShell>;
};
...
export const AdminDashboard = ({ session, logout }) => {
  ...
  return <AdminShell session={session} logout={logout} title="Dashboard Overview" status={error ? { type: "error", text: error } : null}><p className="subtle">Welcome back, Admin. Here's what's happening today.</p><div className="row-between"><div className="subtle">Realtime socket: {connection}</div><button className="btn back-btn" type="button" onClick={reload}>Refresh</button></div><div className="stats-grid four riding-grid"><StatCard icon="US" hint="Live" hintClass="up" title="Total Users" value={metrics.totalUsers ?? 0} /><StatCard icon="HM" tone="green" hint="Live" hintClass="up" title="Active Helmets" value={metrics.onlineHelmets ?? 0} /><StatCard icon="RD" tone="green" hint="Live rides" hintClass="up" title="Riding Mode Active" value={metrics.activeRides ?? 0} /><StatCard icon="IN" tone="red" hint="Today" hintClass="down" title="Incidents Today" value={recentAccidents.length} /><StatCard icon="RS" tone="purple" hint={connection} hintClass="down" title="Realtime Status" value={connection === "connected" ? "Connected" : ""} /></div><div className="split-panels"><div className="panel-card"><h2>User Growth</h2><SimpleBarChart items={userBars} /></div><div className="panel-card"><h2>Incident Distribution</h2><SimpleBarChart items={incidentBars} /></div></div><div className="split-panels"><div className="panel-card list-panel"><h2>Recent Users</h2>{recentUsers.length ? recentUsers.map((user) => <div className="list-row" key={user._id || user.email}><div className="row-user"><span className="avatar-small">{(user.fullName || "").slice(0, 1).toUpperCase()}</span><div><strong>{blank(user.fullName)}</strong><small>{blank(user.email)}</small></div></div><span className={`status-pill ${user.isActive ? "ok" : "neutral"}`}>{user.isActive ? "Active" : "Inactive"}</span></div>) : null}</div><div className="panel-card list-panel"><h2>Recent Incidents</h2>{recentAccidents.length ? recentAccidents.map((incident) => <div className="incident-row" key={incident._id}><div><strong>{blank(incident.user?.fullName || incident.helmetId)}</strong><small>{incident.location?.mapUrl ? "Location captured" : ""}</small><small>{incident.createdAt ? new Date(incident.createdAt).toLocaleString() : ""}</small></div><span className={`status-pill ${incident.severity === "severe" ? "danger" : incident.severity === "medium" ? "warn" : "ok"}`}>{blank(incident.responseStatus || incident.severity)}</span></div>) : null}</div></div></AdminShell>;
};
